...
class BERTClsDataloader(Dataset):
    ...
    def __init__(self, case, bertType):
        ...
        if 'roberta' in str(bertType):
            print(bertType)
            #"textattack/roberta-base-ag-news"
            self.tokenizer=RobertaTokenizer.from_pretrained("./MLM_model/roberta-base")
            print('_______________load roberta tokenizer_______________')
        elif bertType:
            self.tokenizer = AutoTokenizer.from_pretrained(bertType)
    ...
    def transform_text_notype(self, data, max_seq_length=256, labels=None,
                           batch_size=32, shuffle=False):

        # data contain list[tuple(text1, text2)]
        # transform data into seq of embeddings
        input_ids, attention_masks = self.convert_examples_to_features_notype(
            data, max_seq_length)
        ...
        return datasetloader
    def convert_examples_to_features_notype(self, examples, max_seq_length):
